...
class MonthlyProfitReportScreen extends StatefulWidget {
  ...
  const MonthlyProfitReportScreen({Key? key, required this.token}) : super(key: key);

  @override
  _monthlyProfitReportScreenState createState() => _monthlyProfitReportScreenState();
}

class _monthlyProfitReportScreenState extends State<MonthlyProfitReportScreen> {
  late List<Map<String, dynamic>> _monthlyProfitReportData = [];
  ...
  void initState() {
    super.initState();
    _fetchMonthlyProfitReportData();
  }

  Future<void> _fetchMonthlyProfitReportData() async {
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      final response = await http.get(
        Uri.parse('${MonthlyProfitReportScreen.baseUrl}profit/profit-monthly/'),
        headers: {'Authorization': 'Bearer ${widget.token}'},
      );

      if (response.statusCode == 200) {
        setState(() {
          _monthlyProfitReportData = List<Map<String, dynamic>>.from(json.decode(response.body)['data']);
        });
      } else {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: 'MonthlyProfit'), // Pass the title dynamically
      drawer: AppDrawer(currentRoute: '/monthlyProfits'),
      body: Column(
          ...
          Expanded(
            child: _isLoading
                ...
                : _errorMessage.isNotEmpty
                    ...
                    : _monthlyProfitReportData.isNotEmpty
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: DataTable(
                                ...
                                columns: [
                                  ...
                                  DataColumn(label: Text('Other Expense')),
                                  DataColumn(label: Text('Profit')),
                                ],
                                rows: _monthlyProfitReportData.asMap().entries.map<DataRow>((entry) {
                                  ...
                                  return DataRow(
                                    cells: [
                                      ...
                                      DataCell(Text('${profit['total_revenue']}')),
                                      DataCell(Text('${profit['other_expences']}')),
                                      DataCell(Row(
                                        mainAxisAlignment: MainAxisAlignment.end,
                                        children: [
                                          Icon(
  ...
  void _applyFilters() async {
    ...
    try {
      ...
      final Uri uri = Uri.parse('${MonthlyProfitReportScreen.baseUrl}profit/profit-monthly/')
